...
| `validate` | 验证配置格式与内容 |
| `validate --doctor` | 验证配置 + 深度健康检查（`doctor` 已废弃） |
...
| `validate --doctor` | 检查 | 验证配置 + 深度健康检查 |
| `doctor` | 检查 | **[Deprecated]** 使用 `validate --doctor` |
...
```
marrow validate [--config PATH] [--doctor]
```

验证 runtime config 文件格式和内容，打印配置摘要。不启动任何服务。

- `--doctor`：在验证之后额外执行深度健康检查（工作目录结构、context 脚本可执行权限、agent 命令可用性）。替代旧的 `doctor` 子命令。

### `marrow doctor` _(Deprecated)_

> **已废弃**：请改用 `validate --doctor`。

```
marrow doctor [--config PATH]
```

检查工作目录结构、所需命令可用性和运行时健康状态。调用时会打印废弃警告。
